...
public class PostOrderActivity extends BaseActivity implements OnClickListener{
	...
	private void findViewById() {
		...
		rl_coupon_main = (RelativeLayout) findViewById(R.id.post_order_rl_coupon_main);
		...
		tv_coupon_use = (TextView) findViewById(R.id.post_order_tv_coupon_num);
	...
	private void setView(OrderEntity orderEn) {
		if (orderEn != null) {
			...
			if (!StringUtil.isNull(orderEn.getPriceCoupon())) {
				tv_coupon.setText("-" + orderEn.getPriceCoupon());
			}
			...
			if (!StringUtil.isNull(couponId) && !"0".equals(couponId)) {
				tv_coupon_use.setText(getString(R.string.coupon_used_sum, orderEn.getPriceCoupon()));
			}else {
				tv_coupon_use.setText("");
			}
			// 发票信息
	...
	public void onClick(View v) {
		...
		case R.id.post_order_rl_pay_type:
			...
			break;
		case R.id.post_order_rl_coupon_main:
			if (!isSuccess) return;
