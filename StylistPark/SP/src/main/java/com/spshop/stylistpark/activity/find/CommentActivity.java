...
public class CommentActivity extends BaseActivity implements OnClickListener {
	...
	private void sendCommentTxt(){
		...
		if (StringUtil.isNull(commentStr)) {
			CommonTools.showToast(getString(R.string.find_comment_input), 1000);
			return;
		}
		startAnimation();
				...
				if (result != null) {
					...
					if (baseEn.getErrCode() == 0) {
						...
						if (StringUtil.isNull(baseEn.getErrInfo())) {
							CommonTools.showToast(getString(R.string.find_comment_ok), 2000);
						}else {
	...
	protected void stopAnimation() {
		...
		refresh_lv.onPullUpRefreshComplete();
		if (lv_show.size() == 0) {
			tv_no_data.setText(getString(R.string.loading_no_data, getString(R.string.find_comment)));
	...
	private void setCommentTitle(int total) {
		setTitle(getString(R.string.find_comment_total, total));
	}
