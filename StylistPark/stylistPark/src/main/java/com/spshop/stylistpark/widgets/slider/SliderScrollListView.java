...
public class SliderScrollListView extends ListView {
	...
	public SliderScrollListView(Context context) {
		super(context);
	}

	public SliderScrollListView(Context context, AttributeSet attrs) {
		super(context, attrs);
	}

	public SliderScrollListView(Context context, AttributeSet attrs, int defStyle) {
		...
		case MotionEvent.ACTION_MOVE:
			if (mPosition != -1) {
				if (Math.abs(mY - y) < 30 && Math.abs(mX - x) > 20) {
					...
					//mFocusedItemView = (SlideView) getItemAtPosition(mPosition);
