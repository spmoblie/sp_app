...
public class RangeBar extends View {
    ...
    private int maxAmount=-1, leftValue=-1,  rightValue=-1;
    private float leftPos=-1f,rightPos=-1f;
...
//        leftRightMarginPx= mLeftThumb.getHalfWidth();
//        barWidthPx=w-2* leftRightMarginPx;
        ...
        mBar = new Bar(ctx, marginLeft, yPos, barLength, mTickCount, mTickHeightDP, mBarWeight, mBarColor,marginLeft);
       // mBar = new Bar(ctx, 0, yPos, w, mTickCount, mTickHeightDP, mBarWeight, mBarColor,marginLeft);
        ...
        //return current thumbs position
        if(onThumbMoveListener!=null){
        	onThumbMoveListener.onThumbMove(getBarLength(), mLeftThumb.getX()-getMarginLeft() , mRightThumb.getX()-getMarginLeft() );
        }
     ...
     * Trevor: return the barLength that the thumbs can be moved on. this is different from the total bar lenght on screen.
     * @param none
    ...
    private void onActionUp(float x, float y) {
        ...
        if (mLeftThumb.isPressed()) {
            ...
            //releaseThumb(mLeftThumb); //remove auto movement
        	mLeftThumb.release();

		} else if (mRightThumb.isPressed()) {

			//releaseThumb(mRightThumb); //remove auto movement
			mRightThumb.release();

		} else {
			
			/*
			 * remove directly touching bar to adjust the value
			 */
...
//	        }
    ...
    private void moveThumb(Thumb thumb, float x) {
        ...
        if (x < mBar.getLeftX() || x > mBar.getRightX()) {
            // Do nothing.
        	Log.d(TAG, "moveThumb()- out of bar lenght x="+x);
    ...
    public void reset(){
    	mLeftThumb.setX(this.getMarginLeft());
    	mRightThumb.setX(this.getMarginLeft()+this.getBarLength());
    ...
    public void setThumbsPosition(int maxAmount,int leftValue, int rightValue){
    ...
    public void setThumbsPosition(float leftPos,float rightPos){
    ...
    private void calThumbsPosition(){
    	
    	if(leftValue==-1 || rightValue==-1 || maxAmount==-1)
    		return;
    	
    	float barLength=getBarLength();
    	float leftMargin=getMarginLeft();
    	float leftX=(leftValue*barLength)/maxAmount;
    	float rightX=(rightValue*barLength)/maxAmount;
    	mLeftThumb.setX(leftMargin+leftX);
    	mRightThumb.setX(leftMargin+rightX);
    	
    	if(onThumbMoveListener!=null){
        	onThumbMoveListener.onThumbMove(getBarLength(), mLeftThumb.getX()-leftMargin, mRightThumb.getX()-leftMargin);
        }
    	invalidate();
    ...
    private void loadThumbsPosition(){
    	if(leftPos==-1f || rightPos==-1f)
    		return;
    	
    	float leftMargin=getMarginLeft();
    	
    	mLeftThumb.setX(leftPos);
    	mRightThumb.setX(rightPos);
    ...
    public float getLeftThumbsX(){
    	return mLeftThumb.getX();
    }
    public float getRightThumbsX(){
    	return mRightThumb.getX();
    }
    ...
    public static interface OnThumbMoveListener {
